<?php
    ...
    class Client
    {
        ...
        public function registerClient($firstName, $lastName, $email, $address, $mobile)
        {
            ...
            $stmt = $db->prepare("INSERT INTO client (first_name, last_name, email, address, mobile) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $firstName, $lastName, $email, $address, $mobile);
            
            if($stmt->execute()) {
                $stmt->close();
                $checkClients = $db->query("SELECT count('id') AS total FROM client ");

                while($allClients = $checkClients->fetch_assoc()) {
                    if($allClients['total'] >= 1) {
                        $getLastClient = $db->query("SELECT * FROM client ORDER BY id DESC LIMIT 1");
        ...
        public function customerEmailExists($email)
        {
            $conn = new Database();
            $db = $conn->db();
            $query = $db->query("SELECT email FROM client WHERE email = '$email'");
        ...
        public function regClientEmailExists($email, $cus_id)
        {
            ...
            if($query->num_rows == 1) {
                $query2 = $db->query("SELECT email FROM client WHERE email ='$email' AND id = '".$cus_id."'");
        ...
        public function updateClient($firstName, $lastName, $email, $address, $mobile, $cusid)
        {
            ...
            $stmt = $db->prepare("UPDATE client SET first_name = ?, last_name = ?, email = ?, address = ?, mobile = ? WHERE id='".$cusid."'");
            $stmt->bind_param("sssss", $firstName, $lastName, $email, $address, $mobile);
