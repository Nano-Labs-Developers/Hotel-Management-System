...
<?php
    if (isset($_POST['submit'])){
        ...
        if ($hr->checkEmailExists($_POST['email'], $userid)) {
            echo'
                <script>
                    location.replace("settings.php?emailexists=true");
                </script>
            ';
        } else if ($hr->checkUsernameExists($username, $userid)) {
            echo'
                <script>
            ...
            $hr->updateAccount($firstName, $lastName, $email, $username, $password, $userid);
        }
    }
