...
<?php
    if (isset($_POST['submit'])){
        ...
        if ($bartender->checkEmailExists($_POST['email'], $userid)) {
            echo'
                <script>
                    location.replace("settings.php?emailexists=true");
                </script>
            ';
        } else if ($bartender->checkUsernameExists($username, $userid)) {
            echo'
                <script>
            ...
            $bartender->updateAccount($firstName, $lastName, $email, $username, $password, $userid);
